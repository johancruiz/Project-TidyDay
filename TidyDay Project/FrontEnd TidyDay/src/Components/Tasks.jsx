...
function Tasks() {
  ...
  const checkTask = (task) => updateTaskStatus(task, "completed");
  const unCheckTask = (task) => updateTaskStatus(task, "to-do");
            ...
            {tasksTodo.length > 0 ? (
              tasksTodo.map((task) => (
                <div className="todo-item p-3 mb-1" key={task.id}>
                  <div className="row">
                    <div className="col-8">
                      <input type="checkbox" onChange={() => checkTask(task)} />
                      <span
